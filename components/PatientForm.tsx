...
export default function PatientForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Błąd podczas generowania diety:', error)
    } finally {
      setLoading(false)
    }
  }
    ...
    <form onSubmit={handleSubmit} className='bg-white p-6 rounded shadow max-w-2xl mx-auto space-y-4'>
      ...
      <div>
        ...
        <div className='flex flex-wrap gap-4'>
          {['niewydolność nerek', 'niewydolność wątroby', 'cukrzyca', 'nadciśnienie'].map((cond) => (
            <label key={cond}>
              <input
                type='checkbox'
                value={cond}
      ...
      <div>
        <label className='block mb-1'>Region świata (np. Polska, Azja, USA)</label>
      ...
      <button
        ...
        disabled={loading}
      >
        {loading ? 'Generowanie...' : 'Wygeneruj dietę'}
      </button>
